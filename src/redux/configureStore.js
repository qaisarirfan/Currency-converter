...
export const loadThemesFilter = createFilter(themesReducerName, null, ['default']);

export const saveRatesFavFilter = createFilter(conversionReducerName, ['favorite']);
export const loadRatesFavFilter = createFilter(conversionReducerName, null, ['favorite']);

export const saveLangFilter = createFilter(languagesReducerName, ['language']);
export const loadLangFilter = createFilter(languagesReducerName, null, ['language']);

export const saveNavigationFilter = createFilter(navigationReducerName, ['navigation']);
export const loadNavigationFilter = createFilter(navigationReducerName, null, ['navigation']);
...
const configureStore = (initialState = {}) => {
  ...
  const composeWithDevToolsExtension = __DEV__ && window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__;
  const composeEnhancers =
    typeof composeWithDevToolsExtension === 'function' ? composeWithDevToolsExtension : compose;

  const middleware = composeEnhancers(applyMiddleware(thunk, createApiClient(clients), logger));
